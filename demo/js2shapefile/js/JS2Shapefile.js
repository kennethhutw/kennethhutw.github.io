		...
		for (var i = 0; i < openlayersgraphics.length; i++) {
			...
			if (feature.getGeometry().getType() === "Point") {
				...
				for (var j = 0; j < points.length; j++) {
					var point = points[j];
					quasiEsriGraphic.geometry.x = point[0];
					quasiEsriGraphic.geometry.y = point[1];
				...
				var rings = [];
				var pointList = feature.getGeometry().getCoordinates();
				for (var j = 0; j < pointList.length; j++) {
					var ring = [];
					var contour = pointList[j];
	...
	var addGoogleGraphics = function(googlegraphics){
		for (var i = 0; i < googlegraphics.length; i++) {
			...
			if (thisgraphic.geometry.type ==='Point') {
				...
				quasiEsriGraphic.geometry.x = thisgraphic.geometry.coordinates[0];
				quasiEsriGraphic.geometry.y = thisgraphic.geometry.coordinates[1];
				quasiEsriGraphic.geometry.type = "POINT";
				this._pointgraphics.push(quasiEsriGraphic);
			}
			else
				if (thisgraphic.geometry.type ==='Polygon')  {
					var ringsMVC = thisgraphic.geometry.coordinates;
					var numRings = ringsMVC.length;
					quasiEsriGraphic.geometry.rings = [];
					for (var r = 0; r < numRings; r++) {
						var ringArray = [];
						var ringMVC = ringsMVC[r];
						var numVerts = ringMVC.length;
						for (var v = 0; v < numVerts; v++) {
							var vertex = ringMVC[v];
							ringArray.push([vertex[0], vertex[1]]);
						}
					...
					if (thisgraphic.geometry.type ==='LineString')  {
						var pathMVC = thisgraphic.geometry.coordinates;
						var length = pathMVC.length;
						quasiEsriGraphic.geometry.paths = [[]];
						for (var v = 0; v < length; v++) {
							var vertex = pathMVC[v];
							quasiEsriGraphic.geometry.paths[0].push([vertex[0], vertex[1]]);
						}
						quasiEsriGraphic.geometry.type = "POLYLINE";
						this._polylinegraphics.push(quasiEsriGraphic);
					}
